...
Estado: todas las fases pendientes. Esta tarea solo produce documentación. Referencias: [PRD](PRD.md), [TECH_SPEC](TECH_SPEC.md), [DATA_RULES](DATA_RULES.md), [AGENTS](AGENTS.md). Cada fase se solicita y verifica por separado; no empezar la siguiente automáticamente.
...
Las fases preparan primero reglas puras y después persistencia, integración, SQL y web. Cada cierre exige actualizar decisiones y evidencia en SOLUCION sin inventar resultados. Nombres de módulos, comandos, ramas y commits siguientes son propuestas, no archivos o acciones ya ejecutadas.
...
- **Finalización:** servicios sanos y conexión documentada; discrepancia `.env`/Compose entendida; ninguna credencial añadida a Git; comandos reales del entorno en SOLUCION.
- **Commits sugeridos:** `chore: define Python dependencies and test configuration`; `docs: document verified local startup`.
- **Rama / PR:** `codex/etl-products`, PR 1 junto con F1–F4.
...
- **Tests:** tabla de casos de DATA_RULES: encodings, 10%/10/0,1 y 1 ambiguo, importes europeos/anglosajones inequívocos, miles ambiguos rechazados, NaN/rango, EAN válido/checksum/scientific, fechas imposibles/bisiestas, centinelas vs guion dentro de SKU, Decimal y redondeo en medio céntimo. Comprobar que secretos no entran en errores/logs.
...
- **Rama / PR:** `codex/etl-products`, PR 1.
- **Dependencias:** F0.

## F2 — Catálogo de productos

- **Objetivo:** extraer candidatos y problemas de calidad conservando procedencia.
- **Tareas:** lector Latin-1/`;`, cabecera/columnas/quoting, campos obligatorios y opcionales, EAN con descarte de campo, candidatos repetidos por SKU y criterio de ganador; preparar contrato para que F3 valide precio final antes de selección definitiva. No persistir todavía ni modificar data.
- **Archivos previstos:** `src/etl/catalog.py`, `tests/unit/test_catalog.py`, fixtures CSV pequeñas.
- **Tests:** columnas de más/menos, registro multilínea, SKU vacío, opcionales inválidos, duplicados exactos y conflictos, empate por ordinal y procedencia del perdedor; coste inválido queda candidato condicionado a excepción válida de F3, no aceptado como producto comercial por sí solo.
...
- **Tests:** excepción evita todo descuento; coste base inválido con/sin excepción; categoría+marca, solo una/ninguna, texto normalizado, descuentos inválidos o suma >=100%, regla duplicada/conflictiva, excepción huérfana, XML roto. Caso calculado manualmente 100/10%/5% = 85, identificado como fixture ficticia.
...
- **Rama / PR:** `codex/etl-products`, cerrar PR 1 a `main`.
- **Dependencias:** F3.
...
- **Tests:** transporte httpx simulado, reloj/sleep inyectables; varias páginas, vacía, 401 sin retry, 500 recuperable/agotado, 429 con segundos/HTTP-date, timeout, meta inconsistente, página repetida; almacenes duplicados, empate contradictorio, quantity/reserved inválidos, stock ausente vs 0. Fallo de última página conserva toda la versión de negocio anterior.
...
- **Rama / PR:** `codex/etl-stock`, PR 2 a `main`.
...
- **Tests:** cabecera con fecha con/sin hora, vacío recuperable/no recuperable, valores contradictorios, cliente ausente; negativos DEVUELTO vs COMPLETADO, dos precios distintos mismo SKU, firma estable, duplicado idéntico, corrección de línea elimina anterior, pedido ausente retirado, histórico reutilizado/promovido, pedido parcial y pedido sin líneas, FKs reales y rollback.
...
- **Rama / PR:** `codex/orders`, PR 3 junto con F7.
...
- **Tareas:** prueba completa con entradas congeladas; reconciliar leídos/aceptados/descartados e incidencias; comparar valores y conteos de negocio entre ejecuciones, excluyendo auditoría; fallo durante extracción y durante carga; verificar política de concurrencia. Registrar evidencias reales en SOLUCION.
...
- **Tests:** cero huérfanos, UNIQUEs, stock suma válida, coste neto esperado, hashes/conjuntos equivalentes, auditoría nueva en segunda ejecución, rollback sin estado mixto, run interrumpido recuperable. Con fuentes reales no forzar conteos de fixtures; observar y registrar los reales.
...
- **Rama / PR:** `codex/orders`, PR 3 a `main`.
- **Dependencias:** F6.
...
- **Rama / PR:** `codex/dashboard`, PR 4 junto con F9.
...
- **Rama / PR:** `codex/dashboard`, PR 4 a `main`.
- **Dependencias:** F8.
...
- **Tests:** resumen coincide con run/SQL, serialización Decimal, timeout/500/fallo sin rollback del ETL, mismo run_id al reintentar; ejecución real verifica histórico y alerta cuando corresponde, incluida prueba controlada de umbral marcada como tal. HTTP 2xx no basta para acreditar destinos.
...
- **Rama / PR:** `codex/make-integration`, PR 5 a `main`.
...
- **Rama / PR:** `codex/optional-hardening`, PR 6 opcional; dividir por extra si crece.
...
- **Tareas:** completar SOLUCION; validar arranque desde clon limpio y BD de pruebas nueva sin borrar volumen del usuario; ETL dos veces con entradas controladas; tests/lint configurado; dashboard y Make; revisar secretos en cambios, historial relevante, blueprint y capturas; `.gitignore`; verificar ≥3 ramas, ≥2 PR y main funcional. Explicar cinco millones de líneas, limitaciones, exclusiones y uso de IA. Preparar borrador de correo y enlaces/acceso, sin enviarlo automáticamente.
...
- **Rama / PR:** `codex/delivery-docs`, PR final a `main`.
...
| --- | --- | --- |
| 1 | `codex/etl-products` | F0–F4: catálogo y tarifas con MySQL/auditoría y pruebas. Abrir borrador temprano; incorporar commits por fase, sin esperar a todo el proyecto. |
| 2 | `codex/etl-stock` | F5: cliente y consolidación de stock con fallos controlados. |
| 3 | `codex/orders` | F6–F7: cuatro fuentes relacionadas, pruebas E2E e idempotencia. |
| 4 | `codex/dashboard` | F8–F9: SQL validado y panel completo. |
| 5 | `codex/make-integration` | F10: integración real y artefactos Make. |
| 6 opcional | `codex/optional-hardening` | F11: extras seleccionados. |
| Final | `codex/delivery-docs` | F12: guía y evidencias finales. |
...
La siguiente tarea recomendada es F0: validar entorno y fijar dependencias sin implementar todavía extractores. Resolver cada supuesto en su fase y dejar documentada la decisión si no hay confirmación comercial; nunca rellenar huecos con datos inventados.
