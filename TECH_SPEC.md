...
4. Confirmar datos y estado/resumen del run conjuntamente. Si falla MySQL, rollback de negocio y marcar run failed aparte. En caída abrupta, el siguiente arranque debe detectar el run running sin proceso activo y registrarlo como interrumpido antes de seguir.
...
D: usar el precio de la excepción XML válida cuando exista; en su ausencia, coste × (1 − descuento categoría − descuento marca). La suma debe ser menor que 1 para producir coste positivo. Alternativa secuencial: coste × (1−dc) × (1−dm); confirmar semántica antes de cambiar. No aplicar `PorVolumen` por faltar cantidad de compra ni portes/plazos al coste; no son requisitos y no se pueden deducir de cantidades vendidas. Reglas repetidas idénticas se deduplican; contradictorias invalidan la clave y rechazan productos dependientes sin excepción válida. XML ilegible aborta el run.
...
El total físico usa `sum(quantity)` de un registro vigente por `(sku, warehouse)`; reservado se conserva aparte. No restar reserved sin decidir que se desea disponibilidad. Si no hay stock observado para un producto, total NULL. Una respuesta completa vacía implica stock desconocido para todos, no cero.
...
D: resumen posterior al commit con `schema_version`, `run_id`, estado/fecha, productos válidos de catálogo presentes en el lote (no affected_rows de MySQL), insertados/actualizados/sin cambio por separado, filas rechazadas únicas, incidencias por motivo, facturación del último mes y lista/conteo de bajo stock. Mismo run_id al reintentar entrega; resumen guardado en etl_runs para reenviarlo sin repetir ETL.

Escenario: webhook -> validación/filtro -> router con ruta de histórico siempre a Google Sheets y ruta condicional de email si `rejected_rows > umbral` O existen productos bajo mínimos. Si no hay incidencias solo corre histórico; ambas rutas corren cuando hay problema. Umbral absoluto entero configurable, inicialmente 0 como propuesta. No hacer las dos rutas mutuamente excluyentes dejando alertas sin histórico. Airtable es alternativa si no se dispone de Google.
